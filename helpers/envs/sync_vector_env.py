...
# I have added this implementation from gymnasium, while they haven't upgraded version.
# It's needed as I noticed that usually Sync vector is not doing autoreset in same step for the env and this basically creates huge waste of data for training in off policy methods for robotic like tasks and makes learning for TD, SAC and others very slow.
# I suspect I had same issues with PPO, and will be doing review all the code for training different methods.
# I like in pull request new implementation letting decide for user if we should have or not reset in sync vector.
# 
# Implementation of a synchronous (for loop) vectorization method of any environment.

# This module provides the `SyncVectorEnv` class, which allows for the serial execution of multiple environments. 
# It supports different autoreset modes, enabling users to control whether environments should reset in the same step or not, 
# which can be crucial for efficient training in off-policy methods for robotic tasks.

# Classes:
#     AutoresetMode: Enum representing the different autoreset modes.
#     SyncVectorEnv: Vectorized environment that serially runs multiple environments.

# Example usage:
# # https://github.com/Farama-Foundation/Gymnasium/pull/1227
# 
...
from typing import Any, Callable, Iterator, Sequence, Optional, Union, Dict
...
__all__ = ["AutoresetMode", "SyncVectorEnv", 'make_sync_vec']
...
def make_sync_vec(
    id: Union[str, object],  # object here represents an EnvSpec if available
    num_envs: int = 1,
    wrappers: Optional[Sequence[Callable[[Env], Env]]] = None,
    vector_kwargs: Optional[Dict[str, Any]] = None,
    autoreset_mode: Optional[Union[str, AutoresetMode]] = None,
    **kwargs,
) -> SyncVectorEnv:
    """
    Create a synchronous vectorized environment with the given id (or EnvSpec) and number of environments,
    applying the provided wrappers to each environment and passing additional vectorization arguments.
    
    Args:
        id: The environment id (as a string) or an environment specification.
        num_envs: Number of environments to create.
        wrappers: An optional sequence of wrapper functions to apply to each created environment.
        vector_kwargs: Optional dictionary of additional keyword arguments for the vectorized environment.
        autoreset_mode: Optional autoreset mode to pass to the vectorized environment.
            This can be a string (e.g., "NextStep", "SameStep", "Disabled") or an instance of AutoresetMode.
        **kwargs: Additional keyword arguments to pass to gymnasium.make when creating each environment.
    
    Returns:
        An instance of SyncVectorEnv with the specified settings.
    """
    wrappers = wrappers or []
    vector_kwargs = vector_kwargs or {}
    
    # If an autoreset mode is provided, include it in the vectorization arguments.
    ...
    def create_single_env() -> Env:
        try:
            ...
            raise Error(f"Error creating environment from id/spec {id}: {e}")
        # Apply each wrapper sequentially
        for wrapper in wrappers:
            env = wrapper(env)
        return env

    # Create a list of environment-creating callables
    env_fns = [create_single_env for _ in range(num_envs)]
    
    # Create and return the synchronous vectorized environment with the extra arguments.
