...
def weights_init_(m):
    if isinstance(m, nn.Linear):
        torch.nn.init.xavier_uniform_(m.weight, gain=1) # xavier_normal_
...
class Actor(nn.Module):
    def __init__(self, env, use_weights_init=False):
        ...
        self.fc_mu = nn.Linear(256, env.action_space.shape[0])
...
if __name__ == "__main__":
    ...
    actor_params, _, _ = torch.load('my_runs/Humanoid-v5__TD3_weightsFalse_1737119844/torch.model', map_location=device)
